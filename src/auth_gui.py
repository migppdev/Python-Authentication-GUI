...
class App(tk.Tk):
    def __init__(self):
        ...
        # Label clickable para cambiar entre modos (registro e inicio de sesion)
        ...
        # Hace clickable el label
        self.change_mode_lbl.bind("<Button-1>", self.show_log_in_gui)
        
    def register_user_gui(self, event=None):
        # Obtener usuario y contraseña y guardarlos en variables
        username = self.username.get()
        password = self.password.get()

        # Intentar registrar el usuario y guardar el resultado de la operacion 
    ...
    def log_in_redirect(self, username):
        # Redirigir a una nueva ventana si el inicio de sesion es correcto
        self.on_closing()
    ...
    def show_log_in_gui(self, event=None):
        # Insertar modal de inicio de sesión en la ventana
    ...
    def show_sign_in_gui(self, event=None):
        # Insertar modal de registro de usuario en la ventana
        self.title("Registro")
        self.username.focus_set()
        self.username.delete(0, tk.END)
        self.password.delete(0, tk.END)
        self.send_btn.config(text="Registrarse")
        self.send_btn.config(command=self.register_user_gui)
        self.change_mode_lbl.config(text="¿Ya estas registrado? Iniciar sesión")
        self.change_mode_lbl.bind("<Button-1>", self.show_log_in_gui)

    def on_closing(self):
        # Cerrar de forma segura la base de datos al cerrar la ventana
        close_db()
        self.destroy()
